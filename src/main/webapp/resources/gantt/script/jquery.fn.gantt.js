...
//해야할 일 : 
//2. 표현될 데이터의 구성요소 뽑고 그것을 DB와 연동시키기
//3. popup기능 추가
//4. 클릭시 detail띄우는거
//6. 빈곳 클릭시 나오는 기능 삭제
//7. 프로젝트 생성날 부터 보이고 켤때는 오늘을 기준으로 후에있는 것 부터 보여준다. 이미 지나간 것들은 뒤로 보낸다.(크게 보이는 기능도 있으면 좋다.)
//날짜는 컨트롤단에서 막아준다. 프로젝트생성시 그 날짜를 기준으로 하나 task를 임의로 만들어 준다.
/*jshint shadow:true, unused:false, laxbreak:true, evil:true*/
/*globals jQuery, alert*/

//$.이름 은 일종의 전역 메소드이다.
//$.fn.이름 = $("#id").이름 확장 집합 메소드라고 하며 확장집합에서 동작하는 객체인스턴스의 메소드이다.
//기본값을 변경할 수 있게 하려면 기본값에 대한 정의를 플러그인 메소드 밖에서 작성하여 플로그인 밖에서도 접근할 수 있는 곳에 위치시켜야 한다.
/*
 * $('a').css("color", "red"); = $.fn.red = function() {
 * 	this.css("color", "red");
 * }
 * 플러그인 호출
 * $('a').red();
...
/*
 * this와 $(this)의 차이
 * -> this는 이벤트가 일어난 그 자신 , $(this)는 this가 가리키는 요소를 감싼 jQuery객체
 */
...
(function ($) {
    ...
    $.fn.gantt = function (options) {
        ...
        // $.extend( obj1 , obj2 );는 obj2를 obj1에 병합한다는 이야기이다.
        // 같은이름의 변수는 덮어쓰고 나머지는 추가한다.
        // 매개변수 가장 앞에 'ture'를 추가하면 같은 변수는 병합하고 나머지는 그대로 두고 합친다. 합친 결과는 obj1에 남는다.
        // 대신 {}를 추가하면 빈객체에 병합한 결과를 넣고 나머지 변수는 그대로 이다.
        $.extend($.expr[":"], {
        ...
        // Date prototype helpers (날짜계산해서 출력해주는 친구들이기 때문에 그냥 두고 써야한다.)
        // ======================
        ...
        // 이곳에 core라는 네임스페이스를 생성하고 아래 있는 것들은 core 객체의 메소드가 된다.(core객체에 캡슐화를 시켜버렸다.)
        var core = {
            // Return the element whose topmost point lies under the given point
            // Normalizes for IE
        	//$.browser.msie를 지워야한다.
        		//이것은 core객체의 메소드 중 하나가 된다. $.core.elementFromPoint(x,y) 형태로 사용한다.
            ...
            // **Create the chart**
            // 이 메소드는 가장 마지막에 실행된다.(this)가 element로 들어온다.
            ...
            render: function (element) {
                ...
                //element = this 이고 leftPanel을 실행시켜서 return된 것을 담음
                content.append($leftPanel);
                //content html껍데기 안에 leftPanel이 계산된 것을 쓴다.
            ...
            leftPanel: function (element) {
                /* Left panel */
                var ganttLeftPanel = $('<div class="leftPanel" style="width:110px"/>')
                    .append($('<div class="row spacer" style="width:110px"/>')
            ...
            navigation: function (element) {
                    ...
                    ganttNavigate = $('<div class="navigate" />')
                        .append($('<div class="nav-slider" />')
                            .append($('<div class="nav-slider-left" />')
                                .append($('<span role="button" class="nav-link2 nav-page-back" />')
                                    .html('&lt;')
                                    .click(function () {
                                ...
                                .append($('<span role="button" class="nav-link2 nav-now"/>')
                                    .html('&#9679;')
                                    .click(function () {
                                        core.navigateTo(element, 'now');
                                    }))
                                .append($('<span role="button" class="nav-link2 nav-prev-week"/>')
                                    .html('&lt;&lt;')
                                    .click(function () {
                                ...
                                .append($('<span role="button" class="nav-link2 nav-prev-day"/>')
                                    .html('&lt;')
                                    .click(function () {
                                        if (settings.scale === 'hours') {
                            ...
                            .append($('<div class="nav-slider-right" />')
                                .append($('<span role="button" class="nav-link2 nav-next-day"/>')
                                    .html('&gt;')
                                    .click(function () {
                                        if (settings.scale === 'hours') {
                                            core.navigateTo(element, tools.getCellSize() * -4);
                            ...
                            .append($('<span role="button" class="nav-link2 nav-next-week"/>')
                                    .html('&gt;&gt;')
                                    .click(function () {
                                        if (settings.scale === 'hours') {
                                            core.navigateTo(element, tools.getCellSize() * -8);
                                ...
                                .append($('<span role="button" class="nav-link2 nav-zoomIn"/>')
                                    .html('&#43;')
                                    .click(function () {
                                        core.zoomInOut(element, -1);
                                    }))
                                .append($('<span role="button" class="nav-link2 nav-zoomOut"/>')
                                    .html('&#45;')
                                    .click(function () {
                                        core.zoomInOut(element, 1);
                                    }))
                ...
                // Button navigation is provided by setting `settings.navigation='buttons'`
                    ...
                    ganttNavigate = $('<div class="navigate" />')
                        ...
                        .append($('<span role="button" class="nav-link2 nav-begin"/>')
                            .html('&#124;&lt;')
                            .click(function () {
                        ...
                        .append($('<span role="button" class="nav-link2 nav-end"/>')
                            .html('&gt;&#124;')
                            .click(function () {
            ...
            fillData: function (element, datapanel, leftpanel) {
                ...
                // $.each가 있다면 어떤 객체를 찾았을 때 그것의 첫번째 것에만 적용하는 것이 아니라, 해당하는 다수의 엘리먼트 함수에 적용이 가능하다.
                // 이런것을 전역함수라고 하며 jQuery 유틸리티 메소드 라이브러리에 더 여러가지가 있다.
                $.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
